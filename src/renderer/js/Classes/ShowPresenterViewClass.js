...
class ShowPresenterView extends ShowPresentationBase {
  ...
  _createBackground(slide) {
    ...
    ImgObj.src = `media://${slide.videoFileName}.${slide.videoThumbnailFormat}`;
    return ImgObj;
  }
  ...
  #createSlideSidebarPreview(slide) {
    ...
    slideThumbPreview.src = `media://${slide.videoFileName}.${slide.videoThumbnailFormat}`;
  ...
  constructor(props) {
    ...
    let addSlideBtn = document.querySelector(
      `#${this.#sideBarSlidesContainer.id}>:first-child`
    );

    this._slides.forEach((slide) => {
      this.#sideBarSlidesContainer.insertBefore(
        this.#createSlideSidebarPreview(slide),
        addSlideBtn
      );
    });

    this.#lyricsContainer.replaceChildren(
      ...this.#createLyricsPreview(this._slides[0])
    );
  ...
  changeSlide(number = 1) {
    ...
    let { currentSlide, currentTextSlide, isNewSlide } = super.changeSlide(
      number
    );
    ...
    if (isNewSlide) {
      ...
      this.#lyricsContainer.replaceChildren(
        ...this.#createLyricsPreview(this._slides[currentSlide])
      );
      this.#lyricsContainer.children[currentTextSlide].classList.add(
        "active-text-slide"
      );
      currentActive.classList.add("active");
      currentActive.scrollIntoView(scrollBehaviour);
      this.#sideBarSlidesContainer.children[
        currentSlide - number
      ...
      this.#lyricsContainer.children[currentTextSlide].scrollIntoView(
        scrollBehaviour
      );
      if (this.#lyricsContainer.children.length > 1) {
        this.#lyricsContainer.children[
