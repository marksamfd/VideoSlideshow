...
class SlideManager {
    ...
    constructor(props: SlideManagerProps) {
        console.log(props);
    ...
    addSlide(slide: SlideType, index = Number(this.#current) + 1) {
    ...
    updateTextFont(fontName: string, index = this.#current) {
        this._slides[index].setFontName(fontName);
    }

    updateTextPosition(x: number, y: number, index = this.#current) {
        this._slides[index].setTextPosition({x, y});
    }

    updateSlideText(text: string, index = this.#current) {
        this._slides[index].setText(text);
    }

    updateSlideVideo(videoName: string, index = this.#current) {
    ...
    /**
     *
     * @returns {SlideType|null} The current slide object, or null if there are no slides.
     */
