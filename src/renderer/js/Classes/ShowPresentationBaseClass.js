...
class ShowPresentationBase extends Konva.Stage {
  ...
  _cacheBG(numOfVideos = 3) {
    return this._slides
      .slice(this.#currentSlide + 2, this.#currentSlide + 2 + numOfVideos)
      .map(this._createBackground.bind(this));
  }
  ...
  constructor(props) {
    ...
    this._backgroundObjs = this._slides
      .slice(0, 3)
      .map(this._createBackground.bind(this));
    ...
    this.#simpleText = new Konva.Text({
      ...
      /*
                ...
                */
      fontFamily: this.slides[this.#currentSlide].fontFamily,
      fill: "white",
      id: "text",
    ...
    this.#textBackground = new Konva.Tag({
      fill: "black",
      opacity: this.slides[this.#currentSlide].fontBackground ? 0.5 : 0,
      cornerRadius: 12,
      padding: this.#padding * 10,
    });
  ...
  changeSlide(number = 1) {
    ...
    if (
      this.#currentTextSlide < this.#textSlides.length &&
      this.#currentTextSlide >= 0
    ) {
      ...
      this.#simpleText.text(
        `${this.#textSlides[this.#currentTextSlide]}\u202e`
      );
      ...
      this.#background.setAttr(
        "image",
        this._backgroundObjs[this.#currentSlide]
      );
    ...
    console.log(
      `Text ${this.#currentTextSlide + 1} of ${this.#textSlides.length},`,
      `Slide ${this.#currentSlide + 1} of ${this._slides.length},\n`,
      `Text Height: ${this.#simpleText.getClientRect().y - this.#padding / 2}`,
      `compare to height: ${
        +this.#simpleText.getClientRect().y - this.#padding / 2
      }`
    );
    return {
      currentTextSlide: this.#currentTextSlide,
